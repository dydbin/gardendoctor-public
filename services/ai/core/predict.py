...
MODEL_DIR = Path(os.getenv("AI_MODEL_DIR", "ai_models"))
MODEL_UNAVAILABLE_MESSAGE = "AI 진단 모델이 준비되지 않았습니다."

# --- 1. 작물별 질병 모델 설정 (2단계: Specialist Models) ---
MODEL_CONFIG = {
    "tomato": {
        "model_path": MODEL_DIR / "new_tomato_classifier_ver2.pth",
        "class_names": ['정상', '토마토잎곰팡이병', '토마토황화잎말림바이러스병', '토마토흰가루병'] 
    },
    "strawberry": {
        "model_path": MODEL_DIR / "no_powdery_leaf_strawberry_classifier.pth",
        "class_names": ['딸기꽃곰팡이병', '딸기모무늬병', '딸기열매흰가루병', '딸기잎마름병', '딸기잿빛곰팡이병', '딸기탄저병', '정상']
    },
    "pepper": {
        "model_path": MODEL_DIR / "pepper_classifier_ver1.pth",
        "class_names": ['고추탄저병', '고추흰가루병', '정상'] 
    },
    "pumpkin": {
        "model_path": MODEL_DIR / "best_pumpkin_classifier_811.pth",
        "class_names": ['단호박점무늬병', '단호박흰가루병', '정상']
    },
    "k_melon": {
        "model_path": MODEL_DIR / "best_k_melon_classifier_811.pth",
...
ROUTER_MODEL_CONFIG = {
    "model_path": MODEL_DIR / "new_finetuned_plant_model.pth",
...
def load_router_model():
    ...
    try:
        ...
        return None, "라우터 모델을 불러올 수 없습니다."
...
def load_specialist_model(crop_name: str):
    ...
    try:
        ...
        return None, None, MODEL_UNAVAILABLE_MESSAGE
    except Exception:
        return None, None, "전문가 모델을 불러올 수 없습니다."
