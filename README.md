...
https://github.com/KSEB-AI-3
  ...
  GardenDoctor는 매일 오전 9시에 사용자의 식물 상태를 확인하고, 물주기·가지치기·영양제 작업을 사용자별 알림으로 생성해야 합니다. 데이터 증가 상황을 검증하기 위해 활성 사용자 5만 명 이상을 가정하고, 최종 진단 데이터는 10만 명까지
  확장했습니다.

  초기 구현은 기능적으로는 동작했지만 처리 대상이 증가하면 DB 조회, SQL 실행, transaction과 외부 FCM 호출이 사용자 수에 비례해 증가하는 구조였습니다.
  ...
  이 문제는 네트워크 I/O나 디스크 I/O 하나가 느린 것이 아니라, 한 건의 논리적 알림을 처리하기 위해 여러 DB I/O와 transaction, 외부 I/O를 사용자 수만큼 반복하는 I/O 증폭 구조였습니다.

  #### 2. 원인 분석

   원인                    기존 구조의 문제                                                                                                                                                                                         
  ━━━━━━━━━━━━━━━━━━━━━━  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   계산형 조회 조건        DATEDIFF(CURRENT_DATE, last_date)를 row마다 계산하여 일반 B-Tree 인덱스로 range scan하기 어려움
  ──────────────────────  ─────────────────────────────────────────────────────────────────────────────────────────────────────────
   OFFSET 페이지 이동      실행 중 탈퇴나 알림 설정 변경으로 대상 집합이 달라지면 뒤 페이지가 이동하여 중복·누락 가능
  ──────────────────────  ─────────────────────────────────────────────────────────────────────────────────────────────────────────
   사용자별 transaction    User 조회, Notification 저장과 Outbox 저장을 사용자마다 commit하여 SQL과 transaction 수가 선형 증가
  ──────────────────────  ─────────────────────────────────────────────────────────────────────────────────────────────────────────
   JPA 단건 저장           saveAll()도 IDENTITY 키 전략에서는 자동으로 JDBC batch가 되지 않으며 entity materialization 비용이 남음
  ──────────────────────  ─────────────────────────────────────────────────────────────────────────────────────────────────────────
   식물 단위 처리          한 사용자가 여러 식물을 보유하면 같은 사용자에게 여러 개의 관리 알림이 생성될 수 있음
  ──────────────────────  ─────────────────────────────────────────────────────────────────────────────────────────────────────────
   외부 호출 결합          FCM 응답을 기다리는 동안 DB connection을 점유하고, DB commit과 푸시 성공의 부분 실패를 복구하기 어려움
  ──────────────────────  ─────────────────────────────────────────────────────────────────────────────────────────────────────────
   다중 인스턴스           각 인스턴스의 @Scheduled가 동시에 실행되면 동일 알림이 중복 생성될 수 있음

  5,000건 비교 테스트에서는 외부 FCM을 제외하고 DB 저장 경로만 분리해 측정했습니다. 기존 구조를 재현한 경로는 미리 선정한 사용자 ID마다 User를 다시 조회하고 Notification과 Outbox를 저장하여 5,000개의 transaction을 실행했습니다.
  ...
  ────────────────────────────────────  ─────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────  ──────────────────────────────────
   @Async로 FCM 호출                     scheduler thread와 외부 호출 분리                      프로세스 종료 시 작업 유실, 재시도·처리 이력 부재                                               제외
  ────────────────────────────────────  ─────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────  ──────────────────────────────────
   FCM Topic                             서버의 fan-out 비용 감소                               사용자별 식물명·작업 개인화와 수신자별 결과 추적 불가                                           공통 공지에만 적합
  ────────────────────────────────────  ─────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────  ──────────────────────────────────
   Spring Batch                          JobRepository, checkpoint, restart, retry·skip 제공    FCM 중복과 DB·외부 시스템 정합성을 자동으로 해결하지 못하며 현재 규모에서는 운영 복잡도 증가    운영 checkpoint 요구 전까지 보류
  ────────────────────────────────────  ─────────────────────────────────────────────────────  ──────────────────────────────────────────────────────────────────────────────────────────────  ──────────────────────────────────
  ...
  Spring Batch를 사용하지 않은 이유는 성능 최적화에 필요한 핵심이 프레임워크 도입 자체가 아니라 대상 조회 방식, DB I/O 횟수, transaction 경계와 외부 FCM 분리에 있었기 때문입니다. 실제 운영에서 실패 지점 재시작과 JobRepository 기
  반 checkpoint가 필요해지면 현재 producer를 Spring Batch Job/Step으로 전환할 수 있지만, Outbox와 unique key는 그대로 필요합니다.
  ...
  - 관리 예정일을 next_watering_date, next_pruning_date, next_fertilizing_date로 미리 계산하여 next_due_date <= CURRENT_DATE 형태의 range query로 변경했습니다.
  - 사용자 ID를 기준으로 1,000명씩 keyset 조회하여 OFFSET 이동과 대상 변경에 따른 누락 위험을 제거했습니다.
  ...
  - 일시 오류는 재시도하고, 잘못된 token 같은 영구 오류는 즉시 실패 처리합니다.
  - MySQL Named Lock은 중복 scheduler 실행 시도를 줄이고, event_key unique 제약은 실제 중복 저장을 방지합니다.
  - 정확성의 최종 방어선은 Named Lock이 아니라 DB unique key입니다.

  #### 5. 실행 결과와 측정 범위

  알림 측정값은 모두 같은 경로의 데이터 크기만 변경한 결과가 아닙니다. 각 측정의 포함 범위와 transaction 수가 다릅니다.

   측정                    포함 범위                                                                                                         결과    증거 성격                                                                      
  ━━━━━━━━━━━━━━━━━━━━━━  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   5,000건 기존 DB 경로    선정된 ID를 사용자별로 다시 조회하고 Notification·Outbox 저장, 5,000 transaction                              55,444ms    단일 로컬 관측
  ──────────────────────  ──────────────────────────────────────────────────────────────────────────────────  ────────────────────────────────────  ───────────────────────────────────────────────
   5,000건 개선 DB 경로    같은 대상과 payload를 1,000건씩 JDBC batch 저장, 5 transaction                                   1,009ms, 54.95배 단축    단일 로컬 관측
  ──────────────────────  ──────────────────────────────────────────────────────────────────────────────────  ────────────────────────────────────  ───────────────────────────────────────────────
   100,000건 producer      keyset 조회, 작업 조회·집계, Notification·Outbox 생성, 100 chunk commit                        9.609초, 약 10,407건/초    시간은 단일 관측, 건수·chunk assertion
  ──────────────────────  ──────────────────────────────────────────────────────────────────────────────────  ────────────────────────────────────  ───────────────────────────────────────────────
   100,000건 drain         500건씩 200회 claim, mock FCM 결과, 상태 완료                                                  24.033초, 약 4,161건/초    시간은 단일 관측, 처리 건수·backlog assertion
  ──────────────────────  ──────────────────────────────────────────────────────────────────────────────────  ────────────────────────────────────  ───────────────────────────────────────────────
   전체 DB pipeline        producer와 drain 순차 합계                                                          33.642초, 약 2,972건/초, backlog 0    로컬 DB pipeline 기준

  5,000건 비교는 executeBatch() 호출 시간만 측정한 것이 아니라 transaction commit까지 포함합니다. 다만 기존 경로를 항상 먼저 실행한 단일 측정이므로 JIT, MySQL buffer pool과 host 부하의 영향을 받을 수 있습니다. 따라서 54.95배는
  운영 처리량 보장이 아니라 해당 로컬 실행에서 관측한 개선 폭입니다.

  100,000건 drain의 FCM은 in-memory mock입니다. 실제 Firebase network, quota, 429 응답과 재시도 지연은 포함하지 않습니다. scheduler의 MySQL Named Lock 경로 역시 성능 측정에서 제외했습니다.

  정확한 시간은 회귀 조건으로 고정하지 않았습니다. 자동 테스트는 다음 불변 조건을 강제합니다.
  ...
  관련 구현과 진단은 UserPlantCareJobService (services/backend/src/main/java/com/project/farming/domain/userplant/service/UserPlantCareJobService.java), CareNotificationBatchWriter (services/backend/src/main/java/com/project/
  farming/domain/userplant/service/CareNotificationBatchWriter.java), FcmOutboxProcessor (services/backend/src/main/java/com/project/farming/domain/notification/outbox/FcmOutboxProcessor.java), 알림 성능 통합 진단 (services/
  backend/src/test/java/com/project/farming/domain/userplant/service/UserPlantCareBatchPerformanceIntegrationDiagnosticsTest.java)에서 확인할 수 있습니다.

  ### 전체 리팩토링 검증 요약

   문제                         최종 선택                                                검증 결과와 해석 범위                                                                                                                      
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   객체 연관관계와 삭제 전파    식별자 참조 + 도메인별 Soft Delete + 명시적 삭제         source diagnostic으로 main entity의 JPA 관계 annotation 0개를 강제하고, 삭제·이력 보존 정책을 8개 Docker MySQL 시나리오로 검증
  ───────────────────────────  ───────────────────────────────────────────────────────  ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
   물리 FK 제거 후 정합성       서비스 참조 검증 + 28개 무결성 진단 + 충돌별 row lock    long-lived 로컬 DB에서 과거 FK 16개, fresh schema에서 0개를 관측. 16 → 0은 운영 보장이 아닌 로컬 schema 진단 결과
  ───────────────────────────  ───────────────────────────────────────────────────────  ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
   Diary N+1                    페이지 조회 + 연결 ID·이미지 IN 일괄 조회                수정 전 6건에서 13 queries를 관측. 현재 회귀 테스트는 1건에서 30건으로 증가해도 query 증가량 ≤ 1, 전체 ≤ 5를 강제하며 최신 로컬 실행은 3 →
                                                                                         3 queries
  ───────────────────────────  ───────────────────────────────────────────────────────  ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  ...
  ───────────────────────────  ───────────────────────────────────────────────────────  ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
   Refresh Token 재사용         SHA-256 fingerprint + 조건부 1-row rotation              raw token을 저장하지 않고, 동일 old token을 사용한 동시 2요청에서 성공 1건·거부 1건을 MySQL 통합 테스트로 검증

  Keyset 수치는 인증, Spring Security, Controller, DTO 변환, JSON 응답과 로컬 Docker MySQL을 포함한 HTTP 지연시간입니다. 같은 deep page를 반복 조회한 warm steady-state 결과이며 운영 환경의 cold-cache latency나 SLO로 해석하지 않
  습니다. 개별 k6 원본 결과는 공개 저장소에 포함하지 않고 aggregate baseline (infra/loadtest/baselines/diary-read-local.json)만 보존했습니다.
